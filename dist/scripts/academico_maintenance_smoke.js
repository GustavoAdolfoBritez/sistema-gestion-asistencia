...
const academico_service_1 = require("../src/modules/academico/academico.service");
...
async function ensureSampleData() {
    ...
    try {
        ...
        const { rows: facRows } = await cliente.query(`INSERT INTO facultades (nombre, estado)
             VALUES ($1, TRUE)
             ON CONFLICT (nombre) DO UPDATE SET nombre = EXCLUDED.nombre
             RETURNING id`, ['Facultad Smoke']);
        const facultadId = facRows[0].id;
        const { rows: carrRows } = await cliente.query(`INSERT INTO carreras (facultad_id, nombre, codigo)
             VALUES ($1, $2, $3)
             ON CONFLICT (facultad_id, nombre) DO UPDATE SET codigo = EXCLUDED.codigo
             RETURNING id`, [facultadId, 'Carrera Smoke', 'SMK']);
        const carreraId = carrRows[0].id;
        const { rows: planRows } = await cliente.query(`INSERT INTO planes_estudio (carrera_id, nombre, resolucion)
             VALUES ($1, $2, $3)
             ON CONFLICT (carrera_id, nombre) DO UPDATE SET resolucion = EXCLUDED.resolucion
             RETURNING id`, [carreraId, 'Plan Smoke', 'SMK-2026']);
        const planId = planRows[0].id;
        const { rows: materiaRows } = await cliente.query(`INSERT INTO materias (plan_id, nombre, codigo, carga_horaria)
             VALUES ($1, $2, $3, 60)
             ON CONFLICT (plan_id, codigo) DO UPDATE SET nombre = EXCLUDED.nombre
             RETURNING id`, [planId, 'Materia Smoke', 'SMK-101']);
        const materiaId = materiaRows[0].id;
        const { rows: userRows } = await cliente.query(`INSERT INTO usuarios (nombres, apellidos, email, username, password_hash, estado, permisos_especiales)
             VALUES ($1, $2, $3, $4, $5, 'activo', '{}'::jsonb)
             ON CONFLICT (email) DO UPDATE SET username = EXCLUDED.username
             RETURNING id`, ['Smoke', 'Docente', FALLBACK_EMAIL, FALLBACK_USER, bcryptjs_1.default.hashSync('demo', 8)]);
        const usuarioId = userRows[0].id;
        const { rows: docenteRows } = await cliente.query(`INSERT INTO docentes (usuario_id, legajo, titulo_academico)
             VALUES ($1, $2, $3)
             ON CONFLICT (usuario_id) DO UPDATE SET titulo_academico = EXCLUDED.titulo_academico
             RETURNING id`, [usuarioId, 'SMK-LEG', 'Profesor Smoke']);
...
function pickMonthAvoiding(current) {
...
async function main() {
    ...
    try {
        ...
        const cursoUpd = await (0, academico_service_1.actualizarCurso)(cursoId, { aula: 'Lab 99', cupo: 35 });
        console.log('Curso actualizado:', cursoUpd);
        console.log('--- Cerrar módulo');
        const moduloCerrado = await (0, academico_service_1.actualizarModulo)(moduloId, { estado: 'cerrado' });
        console.log('Módulo actualizado:', moduloCerrado);
        console.log('--- Intentar crear curso con módulo cerrado (debe fallar)');
        try {
            await (0, academico_service_1.crearCurso)({ moduloId, docenteId, aula: 'No debería', horarioInicio: '10:00', horarioFin: '12:00' });
        ...
        if (cursoId) {
            await (0, academico_service_1.eliminarCurso)(cursoId).catch(() => undefined);
        }
        if (moduloId) {
            await (0, academico_service_1.eliminarModulo)(moduloId).catch(() => undefined);
        }
