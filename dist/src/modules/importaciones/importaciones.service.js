...
exports.esLoteDelUsuario = esLoteDelUsuario;
exports.validarCargaAlumnosPrevia = validarCargaAlumnosPrevia;
exports.crearLote = crearLote;
exports.listarLotes = listarLotes;
exports.obtenerDetalleLote = obtenerDetalleLote;
exports.agregarRegistrosLote = agregarRegistrosLote;
exports.listarRegistrosLote = listarRegistrosLote;
exports.actualizarEstadoLote = actualizarEstadoLote;
exports.descartarLotePendienteSinRegistros = descartarLotePendienteSinRegistros;
exports.eliminarLotePendiente = eliminarLotePendiente;
exports.confirmarLote = confirmarLote;
exports.listarDestinosAcademicos = listarDestinosAcademicos;
const database_1 = require("../../config/database");
function mensajeErrorAlumno(error, fallback) {
    if (error instanceof Error && 'code' in error) {
        ...
        if (code === '23503')
            return 'El registro hace referencia a una carrera o facultad que no existe.';
    }
...
async function sincronizarTotalRegistrosLote(cliente, loteId) {
    await cliente.query(`UPDATE lotes_importacion
         SET total_registros = (
...
async function asegurarLoteExiste(loteId) {
    const { rows } = await database_1.pool.query(`SELECT id FROM lotes_importacion WHERE id = $1`, [loteId]);
    if (!rows[0]) {
        throw new Error('Lote no encontrado');
    }
}
/** Indica si el lote existe y fue creado por el usuario (para aislar historial y operaciones por lote). */
async function esLoteDelUsuario(loteId, usuarioId) {
    if (!usuarioId) {
        return false;
    }
    const { rows } = await database_1.pool.query(`SELECT 1 FROM lotes_importacion WHERE id = $1 AND ejecutado_por = $2::uuid`, [loteId, usuarioId]);
    return Boolean(rows[0]);
}
async function validarDestinoAcademico(input) {
    ...
    const { rows } = await database_1.pool.query(`SELECT id, facultad_id FROM carreras WHERE id = $1`, [input.destinoCarreraId]);
...
async function validarCursoDestinoMatriculas(input) {
    ...
    const { rows } = await database_1.pool.query(`SELECT crr.id AS carrera_id, f.id AS facultad_id
         FROM cursos c
         ...
         WHERE c.id = $1`, [input.cursoDestinoId]);
    const curso = rows[0];
    if (!curso) {
...
function semestresLoteCoinciden(a, b) {
    if (a == null && b == null)
        return true;
    if (a == null || b == null)
        return false;
...
async function assertLoteImportacionNoDuplicado(input, usuarioId) {
    const archivo = (input.archivoFuente ?? '').trim();
    if (!archivo) {
        return;
    }
    const { rows: pendientesMismoArchivo } = await database_1.pool.query(`SELECT id
         ...
         ORDER BY id DESC
         LIMIT 1`, [usuarioId, input.tipoLote, archivo]);
    if (pendientesMismoArchivo.length) {
        throw new Error(`Ya hay una carga pendiente de «${archivo}» (lote #${pendientesMismoArchivo[0].id}). Descartala en el historial o confirmala.`);
    }
    ...
    const { rows } = await database_1.pool.query(`SELECT id, estado, descripcion
         FROM lotes_importacion
         WHERE ejecutado_por = $1::uuid
           AND LOWER(TRIM(tipo_lote)) = LOWER(TRIM($2))
           AND LOWER(TRIM(archivo_fuente)) = LOWER(TRIM($3))
           ...
           AND LOWER(TRIM(estado)) = 'completado'`, [usuarioId, input.tipoLote, archivo, input.destinoCarreraId, input.destinoFacultadId ?? null, cohorteSql]);
...
async function validarCargaAlumnosPrevia(input, usuarioId) {
    ...
    try {
        await validarCoherenciaRegistrosAlumnosImportacion(cliente, {
            tipo_lote: 'alumnos',
...
async function crearLote(input, usuarioId) {
    await validarDestinoAcademico(input);
    await validarCursoDestinoMatriculas(input);
    await assertLoteImportacionNoDuplicado(input, usuarioId);
    const cohorteSql = Math.trunc(input.cohorteAnio);
    const { rows } = await database_1.pool.query(`INSERT INTO lotes_importacion (
            tipo_lote,
            descripcion,
            archivo_fuente,
            total_registros,
...
async function listarLotes(filtro = {}) {
    const condiciones = [];
    const valores = [];
    if (filtro.estado) {
...
async function obtenerDetalleLote(loteId) {
    const { rows } = await database_1.pool.query(`SELECT
            l.id,
            l.tipo_lote,
            l.descripcion,
...
async function agregarRegistrosLote(loteId, registros) {
    if (!registros.length) {
    ...
    try {
        ...
        const { rows: loteRows } = await cliente.query(`SELECT tipo_lote, destino_carrera, destino_facultad, destino_carrera_id, descripcion
             FROM lotes_importacion WHERE id = $1`, [loteId]);
        ...
        const { rows: insertedRows } = await cliente.query(`INSERT INTO registros_importacion (lote_id, fila, datos, valido, mensaje_error)
             SELECT $1, u.f, u.d::jsonb, u.v, u.m
             FROM unnest($2::int[], $3::jsonb[], $4::boolean[], $5::text[]) AS u(f, d, v, m)
             RETURNING id, fila`, [loteId, filasArr, datosArr, validoArr, msgArr]);
...
async function listarRegistrosLote(loteId, filtro = {}) {
    await asegurarLoteExiste(loteId);
    ...
    valores.push(limit);
    valores.push(offset);
    const { rows } = await database_1.pool.query(`SELECT id, fila, datos, valido, mensaje_error
         FROM registros_importacion
         WHERE ${condiciones.join(' AND ')}
         ORDER BY COALESCE(fila, 0), id
         LIMIT $${valores.length - 1}
         OFFSET $${valores.length}`, valores);
    return rows;
}
async function actualizarEstadoLote(loteId, input) {
    await asegurarLoteExiste(loteId);
    const setFragments = [];
    const valores = [];
    if (input.estado) {
    ...
    valores.push(loteId);
    const { rows } = await database_1.pool.query(`UPDATE lotes_importacion
         SET ${setFragments.join(', ')}
         WHERE id = $${valores.length}
         RETURNING id, tipo_lote, descripcion, archivo_fuente, total_registros, procesados, errores, estado`, valores);
    return rows[0];
}
/** Descarta un lote pendiente sin filas cargadas (p. ej. falló la validación tras crear el lote). */
async function descartarLotePendienteSinRegistros(loteId) {
    const { rowCount } = await database_1.pool.query(`DELETE FROM lotes_importacion l
         WHERE l.id = $1
...
async function eliminarLotePendiente(loteId) {
    const { rowCount } = await database_1.pool.query(`DELETE FROM lotes_importacion
         WHERE id = $1 AND LOWER(TRIM(estado)) IN ('pendiente', 'error')`, [loteId]);
    if (!rowCount) {
        throw new Error('No se encontró un lote descartable (solo pendientes o con errores; quizá ya fue confirmado o no existe).');
    }
    return { eliminado: true };
}
function normalizarClaveCampo(valor) {
    return valor
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .trim()
...
function extraerValorDesdeFila(datos, aliases) {
...
function resolverCampoDestino(claveOriginal, config) {
...
function extraerSemestreDesdeDescripcionLoteImport(descripcion) {
    if (!descripcion)
        return null;
...
function normalizarEtiquetaComparacion(valor) {
    return valor
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
...
function textoCompatiblePlanillaVsDestino(planilla, oficial) {
    ...
    if (p === o)
        return true;
    if (p.length >= 3 && (o.includes(p) || p.includes(o)))
        return true;
    ...
    if (hits >= 2)
        return true;
    if (hits === 1 && palabrasO.length <= 3)
        return true;
    return false;
}
function parsearSemestreDesdeTextoPlanilla(valor) {
    if (valor === undefined || valor === null)
        return null;
...
function extraerDeclaracionPlanilla(datos, campo) {
    const clave = campo === 'carrera' ? '_planilla_carrera' : campo === 'facultad' ? '_planilla_facultad' : '_planilla_semestre';
    const v = datos[clave];
 ...
 */
async function validarCoherenciaRegistrosAlumnosImportacion(cliente, lote, registros) {
    if (lote.tipo_lote !== 'alumnos') {
        return;
    }
    const semDestino = extraerSemestreDesdeDescripcionLoteImport(lote.descripcion);
    const declaracionesCarrera = new Set();
    for (const reg of registros) {
        ...
        if (car) {
            ...
            if (lote.destino_carrera && !textoCompatiblePlanillaVsDestino(car, lote.destino_carrera)) {
                throw new Error(`Fila ${reg.fila ?? '?'}: la planilla indica la carrera "${car}" y no coincide con la carrera destino seleccionada "${lote.destino_carrera}".`);
            }
        }
        if (fac && lote.destino_facultad && !textoCompatiblePlanillaVsDestino(fac, lote.destino_facultad)) {
            throw new Error(`Fila ${reg.fila ?? '?'}: la planilla indica la facultad "${fac}" y no coincide con la facultad destino seleccionada "${lote.destino_facultad}".`);
        }
        if (semPla && semDestino != null) {
            const sParsed = parsearSemestreDesdeTextoPlanilla(semPla);
            if (sParsed != null && sParsed !== semDestino) {
                throw new Error(`Fila ${reg.fila ?? '?'}: la planilla indica semestre "${semPla}" y el destino elegido es el ${semDestino}° semestre.`);
            }
        }
    }
    if (declaracionesCarrera.size > 1) {
        throw new Error('La planilla declara más de una carrera distinta en las filas. Revisá el archivo o el destino académico seleccionado.');
    }
    ...
    const { rows: malAsignados } = await cliente.query(`SELECT TRIM(BOTH FROM a.numero_documento) AS doc, c.nombre AS carrera
         FROM alumnos a
         ...
         WHERE TRIM(BOTH FROM a.numero_documento) = ANY($1::text[])
           ...
           AND a.referencia_carrera_id <> $2`, [uniqueDocs, carreraIdDest]);
...
function formatearErrorAlumnosOtraCarrera(conflictos, _carreraDestino) {
...
function prepararPayloadRegistro(datos, config, tipoLote) {
    const payload = {};
    const origen = datos ?? {};
    ...
    if (tipoLote === 'alumnos') {
        ...
        if (!payload.apellidos)
            payload.apellidos = payload.nombre_apellido || 'S/A';
        if (!payload.nombres)
            payload.nombres = payload.nombre_apellido || 'S/N';
...
async function aplicarReferenciaCarreraDesdeLoteAlumnos(cliente, loteId, carreraId) {
    await cliente.query(`
        UPDATE alumnos al
...
function prepararPayloadsRegistros(rows, config, tipoLote) {
    const validas = [];
    const errores = [];
...
async function batchInsertarRegistros(cliente, config, filas) {
    if (!filas.length)
        return { insertados: 0, errores: [] };
    const columnasSet = new Set();
    for (const f of filas)
        for (const col of Object.keys(f.payload))
            columnasSet.add(col);
...
async function insertarRegistrosFilaPorFila(cliente, config, filas, columnas) {
    let insertados = 0;
    const errores = [];
    const conflicto = config.conflictTarget ? `ON CONFLICT ${config.conflictTarget} DO NOTHING` : '';
    for (const f of filas) {
...
function prepararColumnasAlumnos(filas) {
    const columnasSet = new Set();
    for (const f of filas)
        for (const col of Object.keys(f.payload))
            columnasSet.add(col);
...
async function batchInsertarAlumnos(cliente, filas, incluyeCohorte) {
    if (!filas.length)
        return { insertados: 0, errores: [] };
...
async function insertarAlumnosConSavepoint(cliente, validas, incluyeCohorte) {
    return await batchInsertarAlumnos(cliente, validas, incluyeCohorte);
}
async function insertarAlumnosFilaPorFila(cliente, filas, columnas, incluyeCohorte) {
    let insertados = 0;
    const errores = [];
    const setCohorte = incluyeCohorte
        ? `referencia_carrera_id = COALESCE(EXCLUDED.referencia_carrera_id, alumnos.referencia_carrera_id),
...
async function batchResolverAlumnoIds(cliente, filas) {
    const resueltas = [];
    const errores = [];
    const pendientesCi = [];
    const pendientesNombre = [];
    ...
    if (pendientesCi.length) {
        ...
        const { rows: encontrados } = await cliente.query(`SELECT TRIM(BOTH FROM numero_documento) AS numero_documento, id
             FROM alumnos
             WHERE TRIM(BOTH FROM numero_documento) = ANY($1::text[])`, [cis]);
    ...
    if (pendientesNombre.length) {
        for (const p of pendientesNombre) {
            const { rows: candidatos } = await cliente.query(`SELECT id
                 FROM alumnos
                 WHERE LOWER(TRIM(nombres)) = LOWER(TRIM($1))
                   AND LOWER(TRIM(apellidos)) = LOWER(TRIM($2))
                 ORDER BY id
                 LIMIT 2`, [p.nombres, p.apellidos]);
...
async function procesarLoteDestino(loteId, config, tipoLote) {
    const cliente = await database_1.pool.connect();
    try {
        await cliente.query('BEGIN');
        const { rows: loteRows } = await cliente.query(`SELECT destino_facultad_id, destino_carrera_id, descripcion, cohorte_anio
             FROM lotes_importacion
             WHERE id = $1`, [loteId]);
        const contextoLote = {
        ...
        const loteCohorteAnio = loteCohorteAnioRaw != null &&
        ...
        if (tipoLote === 'alumnos' && validas.length) {
            if (contextoLote.destinoCarreraId) {
                const params = [contextoLote.destinoCarreraId];
                ...
                if (!rowCount) {
                    ...
                    if (semestreCurricularDesdeLote != null) {
                        for (const f of validas)
                            f.payload.semestre_curricular = semestreCurricularDesdeLote;
                    }
                    if (loteCohorteAnio != null) {
                        for (const f of validas)
                            f.payload.cohorte_anio = loteCohorteAnio;
                    }
            ...
            if (contextoLote.destinoCarreraId || contextoLote.destinoFacultadId) {
                const cursoIdsUnicos = [...new Set(resueltas
                        .map((f) => Number(f.payload.curso_id))
                        .filter((id) => Number.isFinite(id) && id > 0))];
                if (cursoIdsUnicos.length) {
                    const { rows: cursoRows } = await cliente.query(`SELECT c.id, crr.id AS carrera_id, f.id AS facultad_id
                         FROM cursos c
                         JOIN modulos_academicos ma ON ma.id = c.modulo_id
                         JOIN materias m ON m.id = ma.materia_id
                         JOIN planes_estudio p ON p.id = m.plan_id
        ...
        if (idsValidos.length) {
            await cliente.query(`UPDATE registros_importacion SET valido = TRUE, mensaje_error = NULL WHERE id = ANY($1::int[])`, [idsValidos]);
        }
        if (todosErrores.length) {
            const errIds = todosErrores.map((e) => e.registroId);
            const errMsgs = todosErrores.map((e) => e.mensaje);
            await cliente.query(`UPDATE registros_importacion ri
                 SET valido = FALSE, mensaje_error = u.msg
                 FROM unnest($1::int[], $2::text[]) AS u(id, msg)
                 WHERE ri.id = u.id`, [errIds, errMsgs]);
        }
        ...
        await cliente.query(`UPDATE lotes_importacion
             SET estado = $2,
                 ...
                 errores = $4,
                 ejecutado_en = NOW()
             WHERE id = $1`, [loteId, estadoFinal, procesados, errores]);
        await cliente.query('COMMIT');
...
async function confirmarLote(loteId) {
    ...
    try {
        await cliente.query('BEGIN');
        const { rows } = await cliente.query(`SELECT id, tipo_lote FROM lotes_importacion WHERE id = $1 AND LOWER(TRIM(estado)) = 'pendiente' FOR UPDATE`, [loteId]);
        const lote = rows[0];
        if (!lote) {
            await cliente.query('ROLLBACK');
...
async function listarDestinosAcademicos() {
    const { rows } = await database_1.pool.query(`SELECT
            f.id AS facultad_id,
