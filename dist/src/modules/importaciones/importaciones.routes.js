...
const importacionesApi = (0, express_1.Router)();
importacionesApi.use(...auth_middleware_1.autenticarConPoliticaAlcance, (0, auth_middleware_1.autorizarRoles)(...rbac_1.ROLES_ADMIN_O_ACADEMICOS));
importacionesApi.get('/lotes', async (req, res, next) => {
    try {
        ...
        const lotes = await (0, importaciones_service_1.listarLotes)({
...
importacionesApi.get('/destinos-academicos', async (_req, res, next) => {
    try {
        const catalogo = await (0, importaciones_service_1.listarDestinosAcademicos)();
        res.json(catalogo);
    }
    catch (error) {
        next(error);
...
importacionesApi.post('/validar-carga-alumnos', async (req, res, next) => {
    try {
        ...
        await (0, importaciones_service_1.validarCargaAlumnosPrevia)({
            tipoLote: 'alumnos',
            ...
            cohorteAnio: body.cohorteAnio === '' || body.cohorteAnio === undefined || body.cohorteAnio === null
                ? undefined
...
importacionesApi.post('/lotes', async (req, res, next) => {
    try {
        const { tipoLote, descripcion, archivoFuente, totalRegistros, destinoFacultad, destinoCarrera, destinoFacultadId, destinoCarreraId, cursoDestinoId, cohorteAnio } = req.body ?? {};
        if (!tipoLote) {
        ...
        const lote = await (0, importaciones_service_1.crearLote)({
            tipoLote,
            descripcion,
            archivoFuente,
            totalRegistros,
            ...
            cohorteAnio: cohorteAnio === '' || cohorteAnio === undefined || cohorteAnio === null
                ? undefined
                : Number(cohorteAnio)
        }, usuarioId);
...
importacionesApi.get('/lotes/:loteId', async (req, res, next) => {
    try {
        ...
        if (!(await (0, importaciones_service_1.esLoteDelUsuario)(loteId, usuarioId))) {
            return res.status(404).json({ mensaje: 'Lote no encontrado' });
        }
        const lote = await (0, importaciones_service_1.obtenerDetalleLote)(loteId);
        if (!lote) {
            return res.status(404).json({ mensaje: 'Lote no encontrado' });
        }
        res.json(lote);
...
importacionesApi.get('/lotes/:loteId/registros', async (req, res, next) => {
    try {
        ...
        const registros = await (0, importaciones_service_1.listarRegistrosLote)(loteId, {
...
importacionesApi.post('/lotes/:loteId/registros', async (req, res, next) => {
    ...
    try {
        ...
        const insertados = await (0, importaciones_service_1.agregarRegistrosLote)(loteId, registros);
        ...
        if (loteId) {
            await (0, importaciones_service_1.descartarLotePendienteSinRegistros)(loteId).catch(() => { });
        }
        if (error instanceof Error) {
            return res.status(400).json({ mensaje: error.message });
        }
...
importacionesApi.delete('/lotes/:loteId', async (req, res, next) => {
    try {
        ...
        await (0, importaciones_service_1.eliminarLotePendiente)(loteId);
...
importacionesApi.post('/lotes/:loteId/confirmar', async (req, res, next) => {
    ...
    try {
        ...
        const resultado = await (0, importaciones_service_1.confirmarLote)(loteId);
        await (0, auditoria_service_1.registrarEventoAuditoriaSegura)({
            modulo: 'importaciones',
...
importacionesApi.patch('/lotes/:loteId', async (req, res, next) => {
    try {
        ...
        const actualizado = await (0, importaciones_service_1.actualizarEstadoLote)(loteId, {
            estado,
            procesados,
            errores,
            totalRegistros,
