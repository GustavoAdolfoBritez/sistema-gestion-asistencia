  ...
  return (
    <div className="system-bg text-slate-800 dark:text-[#e7eef9] min-h-screen h-screen overflow-hidden">
      <div className="flex h-full w-full overflow-hidden">
        {sidebarOpen ? (
          <div className="fixed inset-0 bg-black/50 z-20 lg:hidden dark:bg-black/70" onClick={() => setSidebarOpen(false)} />
        ) : null}

        <AppSidebar sidebarOpen={sidebarOpen} onLogout={onLogout} onClose={() => setSidebarOpen(false)} />

        <main className="flex-1 flex flex-col h-full overflow-hidden">
          <header className="flex-shrink-0 h-16 bg-white/95 backdrop-blur-md border-b border-slate-200 flex items-center px-6 gap-3 z-10 dark:bg-[#132a52]/90 dark:border-slate-800">
            <button
              className="lg:hidden text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200"
              onClick={() => setSidebarOpen(true)}
              aria-label="Abrir menu"
            >
              <span className="material-symbols-outlined">menu</span>
            </button>
            <span className="material-symbols-outlined text-blue-600 dark:text-[#6b8bc3]">badge</span>
          ...
          <section className="flex-1 overflow-hidden flex flex-col p-5 gap-4">
            ...
            <div className="relative">
              ...
              <input
                className={`w-full pl-12 pr-12 py-3 rounded-xl border text-sm ${inpList}`}
                placeholder="Buscar alumno por CI, nombre o apellido..."
                value={termino}
                onChange={(e) => setTermino(e.target.value)}
                autoFocus
              />
              {loading ? (
                <span className="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 text-[20px] animate-spin">
                  progress_activity
                </span>
              ) : termino ? (
                <button
                  type="button"
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200"
                  onClick={() => setTermino('')}
            ...
            {esJefeCarrera || !ocultarFiltrosFacultadCarrera ? (
              <div className="rounded-xl border border-slate-200 bg-white/90 px-4 py-3 shadow-sm dark:border-slate-800/80 dark:bg-[#0e1e38]/80 dark:shadow-none">
                <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400 mb-3">
                  <span className="material-symbols-outlined text-[20px] shrink-0">filter_alt</span>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-3 items-end">
                    {!esJefeCarrera ? (
                      <ScopeSelectorSkeleton
                        gridClassName="flex flex-wrap gap-4 flex-1"
                        className="flex-1 min-w-[160px]"
                      />
                    ) : null}
                    <div className="h-10 w-[11rem] rounded-lg bg-slate-200 animate-pulse dark:bg-white/10" />
                  </div>
                ) : (
                <div className="flex flex-wrap gap-x-4 gap-y-3 items-end">
                  {!esJefeCarrera ? (
                  ...
                  <label className="flex flex-col gap-1 w-full min-w-[140px] sm:w-[11rem] sm:min-w-0 shrink-0">
                    ...
                    <AppSelect
                      ...
                      triggerClassName={inpList}
                    />
                  </label>
                  {!esJefeCarrera ? (
                    <button
                      type="button"
                      className="shrink-0 rounded-lg border border-slate-300 bg-white px-4 py-2 text-xs font-medium text-slate-700 shadow-sm hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800/80 dark:text-slate-300 dark:shadow-none dark:hover:bg-slate-700"
                      onClick={() => {
                        setListaFacultadId('');
                        setListaCarreraId('');
            ...
            {/* Layout principal */}
            <div className="flex-1 overflow-hidden grid grid-cols-1 xl:grid-cols-[320px_1fr] gap-4 min-h-0">

              <aside className="rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col overflow-hidden dark:border-slate-800 dark:bg-[#0e1e38] dark:shadow-none">
                <div className="px-4 py-3 border-b border-slate-200 flex items-center gap-2 dark:border-slate-800/60">
                  <span className="material-symbols-outlined text-slate-500 text-[16px] dark:text-slate-400">group</span>
                  <span className="text-xs font-semibold text-slate-600 uppercase tracking-wider dark:text-slate-400">
                    {termino.trim()
                      ? 'Resultados'
                      : hayFiltrosListado
                        ? 'Alumnos filtrados'
                ...
                <div className="flex-1 overflow-y-auto">
                  {resultados.length === 0 && !loading && (
                    <div className="flex flex-col items-center justify-center h-full gap-3 py-10 px-4">
                      <span className="material-symbols-outlined text-slate-300 text-[48px] dark:text-slate-600">
                        person_search
                      </span>
                      <p className="text-sm text-slate-500 text-center dark:text-slate-500">No se encontraron alumnos</p>
                    </div>
                  )}
                  {resultados.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 flex items-center gap-3 transition-all border-l-2 ${
                          isSelected
                            ? 'bg-blue-50 border-blue-500 dark:bg-blue-500/10'
                            : 'border-transparent hover:bg-slate-50 dark:hover:bg-slate-800/30 dark:hover:border-slate-600'
                        }`}
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-slate-500">CI {item.numero_documento}</p>
                          {item.cohorte_anio ? <p className="text-[11px] text-slate-400 dark:text-slate-500">Año ingreso {item.cohorte_anio}</p> : null}
                          <p className="text-[11px] text-slate-500 truncate dark:text-slate-600">
                            {lineaProgramaAlumnoLista(item)}
                          </p>
                        </div>
                        {Number(item.total_matriculas ?? 0) > 0 && (
                          <span className="ml-auto flex-shrink-0 text-[11px] rounded-full bg-slate-200 px-2 py-0.5 text-slate-600 dark:bg-slate-700/60 dark:text-slate-400">
                            {Number(item.total_matriculas)}
                          </span>
                        )}
                      </button>
                    );
                  ...
                  <div ref={sentinelRef} className="h-8 flex items-center justify-center">
                    {loadingMore && (
                      <span className="material-symbols-outlined text-slate-400 text-[18px] animate-spin dark:text-slate-500">
                        progress_activity
                      </span>
                    )}
                  </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col overflow-hidden dark:border-slate-800 dark:bg-[#0e1e38] dark:shadow-none">
                ...
                {!fichaLoading && ficha && (
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="px-5 py-4 border-b border-slate-200 flex items-center gap-4 dark:border-slate-800">
                      <div className="w-12 h-12 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center text-base font-bold text-blue-800 flex-shrink-0 dark:bg-blue-500/15 dark:border-blue-500/30 dark:text-blue-300">
                        {getInitialsAlumno(ficha.alumno)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h2 className="text-lg font-semibold leading-tight truncate text-slate-900 dark:text-[#e7eef9]">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 px-5 py-4 border-b border-slate-200 dark:border-slate-800">
                      {[
                        ...
                        stat.label === 'Justificadas' ? (
                          <button
                            key={stat.label}
                            type="button"
                            onClick={() => setJustificacionesDialogOpen(true)}
                            aria-label="Ver justificaciones presentadas y documentos PDF"
                            className="rounded-xl border border-slate-200 bg-slate-50 p-3 flex items-center gap-3 text-left w-full hover:bg-slate-100 hover:border-slate-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-400/50 dark:border-slate-800 dark:bg-[#132a52]/40 dark:hover:bg-[#1a335c]/80 dark:hover:border-slate-600"
                          >
                          ...
                          <div
                            key={stat.label}
                            className="rounded-xl border border-slate-200 bg-slate-50 p-3 flex items-center gap-3 dark:border-slate-800 dark:bg-[#132a52]/40"
                          >
                            <div className={`p-2 rounded-lg ${stat.bg} flex-shrink-0`}>
                              <span className={`material-symbols-outlined text-[18px] ${stat.color}`}>{stat.icon}</span>
                            </div>
                    ...
                    <div className="px-5 py-3 border-b border-slate-200 dark:border-slate-800">
                      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200/90 bg-gradient-to-r from-slate-50 via-white to-slate-50/80 px-4 py-3 shadow-sm dark:border-slate-700/70 dark:from-[#0c1628] dark:via-[#101d32] dark:to-[#0c1628] dark:shadow-none">
                        <div className="flex min-w-0 items-center gap-3">
                          <div
                            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-blue-200/80 bg-blue-50 dark:border-blue-500/25 dark:bg-blue-500/10"
                            aria-hidden
                          >
                            <span className="material-symbols-outlined text-[20px] text-blue-600 dark:text-blue-300">
                              calendar_month
                            </span>
                          </div>
                          <p className="min-w-0 self-center text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">
                            Periodo académico
                          </p>
                        </div>
                    ...
                    {/* Tabla de trayectoria + promedio destacado */}
                    <div className="flex-1 flex flex-col min-h-0">
                      ...
                      <div className="flex-shrink-0 border-t border-slate-200 bg-slate-50/95 px-4 py-3 dark:border-slate-800 dark:bg-[#0b1827]/90">
                        {promedioTrayectoriaVisible ? (
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex min-w-0 flex-1 items-center gap-3">
                              <div
                                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-[#132a52]/50 dark:shadow-none"
                                aria-hidden
                              >
                              ...
                              <div className="min-w-0">
                                <p className="truncate text-[17px] font-bold text-slate-700 dark:text-slate-200">
                                  Promedio de asistencia {promedioTrayectoriaVisible.anio}
                                </p>
                              </div>
                            </div>
                            <div className="flex flex-col items-start gap-1 pl-12 sm:items-end sm:pl-0 sm:text-right">
                              <span
                              ...
                              {muestraComparacionHistorico ? (
                                <span className="max-w-md text-[11px] leading-snug text-slate-500 dark:text-slate-500">
                                  Año más reciente en historial ({ficha.resumen.anioPromedioAsistencia}):{' '}
                                  {ficha.resumen.promedioPorcentajeAsistenciaMaterias.toFixed(1)}% (
                                  {ficha.resumen.materiasPromedioAnio} materias)
                                </span>
                              ) : null}
                            </div>
                          </div>
                  ...
                  <Dialog open={justificacionesDialogOpen} onOpenChange={setJustificacionesDialogOpen}>
                    <DialogContent className="w-max min-w-[min(100%,20rem)] max-w-[calc(100vw-1.5rem)] gap-0 overflow-x-auto overflow-y-auto rounded-2xl border border-slate-200 bg-white p-0 shadow-xl ring-1 ring-slate-200/70 dark:border-sky-500/25 dark:bg-gradient-to-b dark:from-[#1b355f] dark:to-[#142a4d] dark:shadow-2xl dark:ring-sky-400/15 sm:min-w-[32rem] sm:max-w-[min(96vw,80rem)]">
                      ...
                      <div className="px-4 py-3 sm:px-6 sm:py-4">
                        <div className="max-h-[min(58vh,26rem)] overflow-auto rounded-xl border border-slate-200 bg-slate-50 shadow-inner dark:border-white/10 dark:bg-[#0c1a32]/60">
                                        ...
                                        {pendiente ? (
                                          <div className="flex flex-col gap-2">
                                            ...
                                            <div className="flex flex-wrap gap-1.5">
                                              <button
                                                type="button"
                                                className="inline-flex items-center gap-0.5 rounded-md border border-emerald-700 bg-emerald-700 px-2 py-1 text-[11px] font-semibold text-white shadow-sm hover:border-emerald-800 hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-60 dark:border-emerald-500/50 dark:bg-emerald-600/80 dark:hover:bg-emerald-600"
                                                disabled={resolviendoFila}
                                              ...
                                              <button
                                                type="button"
                                                className="inline-flex items-center gap-0.5 rounded-md border border-rose-700 bg-rose-700 px-2 py-1 text-[11px] font-semibold text-white shadow-sm hover:border-rose-800 hover:bg-rose-800 disabled:cursor-not-allowed disabled:opacity-60 dark:border-rose-500/50 dark:bg-rose-600/80 dark:hover:bg-rose-600"
                                                disabled={resolviendoFila}
